"""Tests for the pump calibration runtime model."""
...
@pytest.mark.parametrize(
    ("model", "a", "b", "c", "duty", "jacobian", "zero_threshold"),
    [
        ("linear", 0.02, -2.0, 0.0, 1000.0, (1000.0, 1.0), 100.0),
        ("dead-zone linear", 0.02, 200.0, 0.0, 1000.0, (800.0, -0.02), 200.0),
        (
        ...
        (
            ...
            (
                1.0 / (1.0 + exp(-1.0)),
                100.0 * (1.0 / (1.0 + exp(-1.0))) * (1.0 - 1.0 / (1.0 + exp(-1.0))) * 500.0,
                -100.0 * (1.0 / (1.0 + exp(-1.0))) * (1.0 - 1.0 / (1.0 + exp(-1.0))) * 0.002,
            ),
            0.0,
        ),
        (
            ...
            0.0,
            1000.0,
            (1000.0**1.5, 1e-5 * 1000.0**1.5 * log(1000.0)),
            0.0,
        ),
    ],
)
def test_model_catalog_metadata_and_mathematics(
    model: str,
    a: float,
    b: float,
    c: float,
    ...
    """Each catalog model keeps its evaluation metadata and derivatives."""
    calibration = Calibration("pump_0", model=model, a=a, b=b, c=c)
    flow = calibration_flow(model, a, b, c, duty)

    assert model in CALIBRATION_MODELS
    assert calibration.named_parameters == dict(
        zip(MODEL_PARAMETER_NAMES[model], (a, b, c), strict=False),
    )
    assert calibration.duty_for(flow) == pytest.approx(duty)
    assert calibration_jacobian(model, a, b, c, duty) == pytest.approx(jacobian)
    assert calibration_zero_threshold(model, a, b, c) == pytest.approx(
        zero_threshold,
    )
    assert calibration.numeric_equation == calibration_equation(model, a, b, c)
    assert calibration_parameter_reason(model, a, b, c) is None


def test_serialized_field_order_and_legacy_defaults_are_stable() -> None:
    """Old JSON records gain only appended fields with unchanged defaults."""
    legacy = {
        "file": "R0_pwm0",
        "a": 0.01,
        "b": -2.0,
    ...
    assert tuple(item.name for item in fields(Calibration)) == (
        "file",
        "a",
        "b",
        "min_duty",
    ...
    assert asdict(calibration) == legacy | {
        "model": "linear",
        "residual": None,
        "fit_points": [],
        "c": 0.0,
